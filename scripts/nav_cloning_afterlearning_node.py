...
class nav_cloning_node:
    def __init__(self):
        ...
        self.path = roslib.packages.get_pkg_dir('nav_cloning') + '/data/修論データ/6262/可視化/通常ガレージ/0,0,0/前失敗/固定障害物２/'#csvの保存先
        self.save_path = roslib.packages.get_pkg_dir('nav_cloning') + '/data/修論データ/6262/可視化/通常ガレージ/0,0,0/前失敗/固定障害物２/'#ptの保存先
        # ディレクトリが存在しない場合は作成する
        os.makedirs(self.path, exist_ok=True)
        os.makedirs(self.save_path, exist_ok=True)
        self.load_path = '/home/ciel/catkin_ws/src/nav_cloning/data/修論データ/6262/可視化/通常ガレージ/0,0,0/model/前失敗/20250722_22:23:48/model_gpu.pt'
    ...
    def loop(self):
        ...
        #ここから画像並べるコード
        # Resize後の画像はfloatなのでuint8に戻す
        img_front_disp = (img * 255).astype(np.uint8)
        img_left_disp = (img_left * 255).astype(np.uint8)
        img_right_disp = (img_right * 255).astype(np.uint8)

        # サイズを統一（念のため）
        img_front_disp = cv2.resize(img_front_disp, (128, 96))  # 表示用は少し大きく
        img_left_disp = cv2.resize(img_left_disp, (128, 96))
        img_right_disp = cv2.resize(img_right_disp, (128, 96))

        # 横に連結（左・前・右の順）
        combined_view = np.hstack((img_left_disp, img_front_disp, img_right_disp))
        #ここまで

        # 表示
        cv2.imshow("3-Camera Combined View", combined_view)
        cv2.moveWindow("3-Camera Combined View", 100, 100)
        cv2.waitKey(1)
        ...
        if self.episode == 100:#モデル生成
            spawn_model_script_path = '/home/ciel/catkin_ws/src/nav_cloning/model_script/my_cylinder/spawn_model.py'#固定スポーン
            #spawn_model_script_path = '/home/ciel/catkin_ws/src/nav_cloning/model_script/my_cylinder/random_spawn_model.py'#ランダムスポーン
