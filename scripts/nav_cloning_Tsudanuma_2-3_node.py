...
class nav_cloning_node:
    def __init__(self):
        ...
        #self.path = roslib.packages.get_pkg_dir('nav_cloning') + '/data/適当/'
        #self.path = roslib.packages.get_pkg_dir('nav_cloning') + '/data/卒論データ/step8500/津田沼2_3/色/半径0.5高さ1/0,1,0/'
        self.path = roslib.packages.get_pkg_dir('nav_cloning') + '/data/卒論データ/step8500/津田沼2_3/通常配置/original/'
    ...
    def loop(self):
        ...
        if self.episode == 9000:#waypoint１周分
        ...
        if self.episode == 12000:#duration２倍にしたから
...
#ここからwaypointの１っ週の終了を知らせる処理の追加        
    # ウェイポイントを読み込む関数
    def load_waypoints(file_path):
        with open(file_path, 'r') as file:
            return yaml.safe_load(file)

# ウェイポイントに到達した際の処理を行う関数
    def reach_waypoint(waypoint):
    # ウェイポイントの座標を取得
        x = waypoint['point']['x']
        y = waypoint['point']['y']
        z = waypoint['point']['z']
...
# 特定のウェイポイントに到達した際に合図を送信
        if (x, y, z) == (-8.44919, 7.79928, 0.00338225):
            subprocess.Popen(['gnome-terminal', '--', 'bash', '-c', 'echo "一周しました"; read -p "Press enter to exit..."'])
    ...
    # ウェイポイントをファイルから読み込む
        waypoints = load_waypoints('/home/ciero/catkin_ws/src/nav_cloning/maps/cit3f_way.yaml')['waypoints']
